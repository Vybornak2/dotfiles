...
log_info "Configuring Alacritty as the default terminal"

if ! command -v alacritty >/dev/null 2>&1; then
  log_warn "alacritty not found; skipping terminal configuration"
  exit 0
fi
...
if gsettings list-schemas | grep -qx "org.gnome.desktop.default-applications.terminal"; then
  gsettings set org.gnome.desktop.default-applications.terminal exec "alacritty" || log_warn "Failed to set GNOME default terminal exec"
  ...
  log_info "GNOME default terminal command set to alacritty"
else
